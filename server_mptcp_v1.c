...
int main(int argc, char* argv[])
{
    ...
    if(isChild){
        ...
        //prepare the mapping arrays and trackers, i.e. set all of them as -1
        for(int i = 0; i < NUM_CHILDREN; i++){
            DSS_pointer[i] = -1;
            Read_pointer[i] = -1;
            for(int j = 0; j < 83; j++){
        ...
        while(1){
            ...
            printf("DSS_seq_num: %d, subflow_seq_num: %d child_num: %d\n",DSS_seq_num,subflow_seq_num,child_num);
            ...
            for(int i = 0; i < NUM_CHILDREN; i++){
                ...
                printf("chucks_to_be_read: %d\n",chunks_to_be_read);
                if(chunks_to_be_read > 0){
                    read_from_child[i] = read(socket_pair_ipc[i][0], data_buf_parent[i], chunks_to_be_read * NUM_BYTES);
                    printf("parent read %d bytes from child %d: %.*s\n",read_from_child[i],i,read_from_child[i],data_buf_parent[i]);
                    if(read_from_child[i] == -1){
                    ...
                    and put it in right position in final output string
                    */
                    int j; //chunk
                    int k = 0; //index of data_buf_parent
                    for(j = 1; j <= read_from_child[i]/NUM_BYTES && j <= chunks_to_be_read; j++){//which chunks to read
                        int l = 0; //starting byte
                        while(l < 4){//(k % 4) || !k){ //loop for each chunk
                            //add it to final string
                            final_output[mapping_arrays[i][Read_pointer[i]+j] + l] = data_buf_parent[i][k];
                            k++;
                            l++;
                        }
                    }
                    // int chunks_read = read_from_child[i] / NUM_BYTES;
                    // for(int chunk = 1; chunk <= chunks_read; chunk++){
                    //     int startByte = mapping_arrays[i][Read_pointer[i] + chunk];
                    //     for(int index = 0; )
                    // }
